...
def theme_prevalence(df: pd.DataFrame) -> pd.DataFrame:
    ...
    counts = (
        ...
        .agg(
            ...
            pct_negative_sentiment=("sentiment_label", lambda s: (s == "negative").mean()),
        )
        .reset_index()
    )
    counts["mention_share"] = counts["mention_count"] / len(df)
    return counts.sort_values("mention_count", ascending=False).round(4)


def theme_impact_vs_baseline(df: pd.DataFrame) -> pd.DataFrame:
    baseline_nps = df["nps"].mean()
    exploded = _explode_themes(df)
    impact = (
        exploded.groupby("theme")
        .agg(
    ...
    impact["csat_gap_vs_brand"] = impact["avg_csat"] - df["csat"].mean()
    impact["revisit_gap_vs_brand"] = impact["avg_revisit"] - df["revisit_intent"].mean()
